...
assert(index.includes("window.LUMINA_LATEX_STAGE='latex-stage17s-lai-insertion-safety-20260521-1'"), 'index should expose Stage 17R');
for (const src of [providerJs, preloadJs]) {
  assert(src.includes("var STAGE = 'stage17s-lai-insertion-safety-1'"), 'provider should expose Stage 17R');
...
(async () => {
  ...
  console.log('Stage 17R compile job 400 direct fallback tests passed.');
})().catch((err) => {
  console.error(err);
  process.exit(1);
});
