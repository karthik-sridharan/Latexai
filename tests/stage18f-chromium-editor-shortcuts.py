...
port = 9286
profile = tempfile.mkdtemp(prefix='chrome-prof-stage18f-')
...
try:
    ...
    result = ev(r'''(() => {
      ...
      const settingsCard = document.getElementById('editorShortcutSettingsCard');
      const hasTokens = !!overlay && overlay.innerHTML.includes('latex-token-command') && overlay.innerHTML.includes('latex-token-env');
      ed.value = 'theorem';
      ed.focus();
      ed.setSelectionRange(0, 7);
      ed.dispatchEvent(new KeyboardEvent('keydown', { key:'b', metaKey:true, bubbles:true, cancelable:true }));
      const envInserted = ed.value === '\\begin{theorem}\n  \n\\end{theorem}' && ed.selectionStart === '\\begin{theorem}\n  '.length;
      ed.value = 'alpha\nbeta';
      ed.setSelectionRange(0, ed.value.length);
      ed.dispatchEvent(new KeyboardEvent('keydown', { key:'[', metaKey:true, bubbles:true, cancelable:true }));
      const commented = ed.value === '% alpha\n% beta';
      ed.setSelectionRange(0, ed.value.length);
      ed.dispatchEvent(new KeyboardEvent('keydown', { key:']', metaKey:true, bubbles:true, cancelable:true }));
      const uncommented = ed.value === 'alpha\nbeta';
      ed.value = '\\begin{proof}';
      ed.setSelectionRange(ed.value.length, ed.value.length);
      ed.dispatchEvent(new KeyboardEvent('keydown', { key:'Enter', bubbles:true, cancelable:true }));
      const smartIndented = ed.value === '\\begin{proof}\n  ' && ed.selectionStart === ed.value.length;
      return {
        overlayExists: !!overlay,
        hasTokens,
        textareaTransparent: getComputedStyle(ed).color === 'rgba(0, 0, 0, 0)',
        settingsCard: !!settingsCard,
        envInserted,
        commented,
        uncommented,
        smartIndented,
        pass: !!overlay && hasTokens && !!settingsCard && envInserted && commented && uncommented && smartIndented
      };
    })()''')
