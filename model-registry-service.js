...
(function () {
  ...
  const STAGE = 'stage19n1q-gemini-openai-any-gpt-model-registry-1';
  ...
  const ROUTES = [
    ...
    { key: 'paper', title: 'Paper-level AI', hint: 'Review, remake, ranking, acceptance improvement', preferredTier: 'fast' },
  ...
  const PROVIDERS = {
    ...
    gemini: {
      ...
      models: [
        { value: 'gemini-2.5-flash', label: 'Gemini · gemini-2.5-flash', tier: 'fast', structuredJson: true, longContext: true, recommendedFor: ['default', 'citation', 'presentation', 'figure', 'slide-repair', 'diagnostic'] },
  ...
  function modelTierGuess(model) {
    ...
    if (/mini|flash|haiku|small|lite/.test(value)) return 'fast';
    if (/pro|sonnet|opus|4\.1$|strong/.test(value)) return 'strong';
    return 'standard';
  }
  ...
  function applyBackendModels(status, modelListing) {
    ...
    providerKeys.forEach((provider) => {
      ...
      next[provider] = {
        ...
        allowedModels: allowed.length ? allowed : models.map((m) => m.value),
        lastUpdated: nowIso()
      };
    });
  ...
  function syncVisibleProviderModel({ repair = false } = {}) {
    ...
    if (customModelEl) {
      ...
      customModelEl.placeholder = providerEl.value === 'openai' ? 'Optional custom GPT model, e.g. gpt-5.4' : providerEl.value === 'gemini' ? 'Optional Gemini model if backend allows it' : 'Optional custom model';
    }
