/* Latexai Stage 4I hard LAI rewrite guard
 * This is intentionally a DOM/editor-level guard:
 * if Copilot's rewrite workflow changes source without \lai{...}, this script
 * rewrites that edit into:
 *
 * % BEGIN LAI-OLD ...
 * % old selected source
 * % END LAI-OLD ...
...
(function () {
  ...
  const STAGE = 'stage4i-hard-lai-guard-1';
  ...
  function isRewriteTask() {
    return task() === 'rewrite-selection-patch';
  }
  ...
  function parseReplacement(raw) {
    ...
    // If the output includes old LAI comments already, keep only the new \lai body when possible.
  ...
  function ensureLaiMacro() {
    try {
      ...
      // Fallback: if active file is a complete TeX document, add the macro there.
      const el = editor();
  ...
  function applyBlock(cap, replacement, reason) {
    ...
    if (/\\lai\s*\{/.test(newText) && /BEGIN\s+LAI-OLD/i.test(newText)) return false;
    ...
    try {
      ...
      const alreadyHasBlock = /%\s*BEGIN\s+LAI-OLD[\s\S]*?\\lai\s*\{/.test(current.slice(Math.max(0, start - 80), Math.min(current.length, end + 1000)));
      if (alreadyHasBlock) return true;

      // If the editor already replaced the selection, replace the inserted diff.
      if (cap.beforeValue && current !== cap.beforeValue) {
        ...
        // If old selection moved, find it.
        const selectedNow = current.slice(start, end);
        if (selectedNow !== oldText) {
      ...
      if (out) out.textContent = `Stage 4I applied with \\lai{...} (${reason}). Old source was commented and the replacement was wrapped.`;
      NS.Main?.toast?.('Stage 4I applied rewrite with \\lai{...}.');
      pending = null;
      return true;
    } finally {
      applying = false;
  ...
  function installEditorPatches() {
    if (!NS.Editor || NS.Editor.__stage4iPatched) return;
    ...
    NS.Editor.__stage4iPatched = true;
  }

  function installCopilotPatch() {
    if (!NS.Copilot || NS.Copilot.__stage4iPatched) return;
    ...
    if (oldAsk) {
      NS.Copilot.askCopilot = async function () {
        ...
        setTimeout(() => enforceFromEditorMutation('after NS.Copilot.askCopilot mutation'), 100);
        return result;
      };
    }
    NS.Copilot.__stage4iPatched = true;
  }

  function installDomHandlers() {
    const el = editor();
    if (el && !el.__stage4iSelectionHandlers) {
      ['select', 'mouseup', 'keyup', 'touchend', 'blur'].forEach((evt) => {
        el.addEventListener(evt, () => {
          const cap = captureSelection(`editor.${evt}`);
          if (cap) lastNonEmptySelection = cap;
        }, true);
      });
      el.__stage4iSelectionHandlers = true;
    }

    const ask = document.getElementById('askCopilotBtn');
    if (ask && !ask.__stage4iAsk) {
      ask.addEventListener('click', () => {
        if (!isRewriteTask()) return;
        rememberSelection('ask-click');
        setTimeout(() => enforceFromOutput('ask-click output 1'), 1200);
        setTimeout(() => enforceFromOutput('ask-click output 2'), 3000);
        setTimeout(() => enforceFromEditorMutation('ask-click mutation'), 1600);
      }, true);
      ask.__stage4iAsk = true;
    }

    ['insertCopilotBtn', 'replaceCopilotBtn', 'applyCopilotPatchBtn'].forEach((id) => {
      const b = document.getElementById(id);
      if (b && !b.__stage4iButton) {
  ...
  function installOutputObserver() {
    const out = document.getElementById('copilotOutput');
    if (!out || out.__stage4iObserver) return;
    ...
    out.__stage4iObserver = obs;
  }

  function installVisibleBadge() {
    const box = document.getElementById('copilotContextChips') || document.getElementById('copilotOutput')?.parentElement;
    if (!box || document.getElementById('laiStage4iBadge')) return;
    const badge = document.createElement('span');
    badge.id = 'laiStage4iBadge';
    badge.textContent = 'LAI Guard Stage 4I active';
    badge.style.cssText = 'display:inline-block;margin:4px 4px 4px 0;padding:3px 7px;border-radius:999px;background:#fee;color:#900;border:1px solid #f99;font:12px system-ui;';
    box.prepend(badge);
  }

  function boot() {
    W.__LATEXAI_STAGE4I_LAI_GUARD_ACTIVE = true;
  ...
  // Modules initialize after DOMContentLoaded, so patch repeatedly for a few seconds.
  let count = 0;
