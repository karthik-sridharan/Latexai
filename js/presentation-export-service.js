/* Latexai Stage 13G PresentationExportService
 * Stage: stage13g-talk-package-validator-zip-1
 *
 ...
 * semantic fields like bullets/latex. Stage 13G normalizes every exported slide
...
(function () {
  ...
  const STAGE = 'stage13g-talk-package-validator-zip-1';
  ...
  let lastTalkPackage = null;
  let lastTalkPackageReport = null;
...
`;
  ...
  function buildTalkManifest(pkg, formats = selectedExportFormats()) {
    const assetFormats = figureAssetFormats();
    const files = [];

    for (const format of formats) {
      const file = pkg.files?.[format];
      if (file) files.push({ role: format, path: file.path, mime: file.mime, bytes: new Blob([file.content || '']).size });
    }

    for (const asset of pkg.figureAssets || []) {
      if (assetFormats.includes('svg')) files.push({ role: 'figure-svg', path: asset.path, mime: 'image/svg+xml', aiGenerated: Boolean(asset.aiGenerated) });
      if (assetFormats.includes('tikz')) files.push({ role: 'figure-tikz', path: asset.tikzPath, mime: 'application/x-tex', aiGenerated: Boolean(asset.aiGenerated) });
    }

    return {
      schema: 'latexai-talk-package-manifest-v1',
      stage: STAGE,
      createdAt: new Date().toISOString(),
      deckTitle: pkg.deck?.deckTitle || '',
      base: pkg.base || '',
      formats,
      figureAssetFormats: assetFormats,
      slideCount: pkg.deck?.slides?.length || 0,
      rootPath: rootPath(),
      validation: pkg.validation?.summary || {},
      files
    };
  }

  function buildTalkReadme(pkg, formats = selectedExportFormats()) {
    const manifest = buildTalkManifest(pkg, formats);
    return [
      `# ${pkg.deck?.deckTitle || 'Latexai Talk Export'}`,
      '',
      `Generated by Latexai ${STAGE}.`,
      '',
      '## Files',
      '',
      ...
      '## Notes',
      '',
      '- JSON is for Presentation Maker import.',
      '- HTML is a standalone printable talk page.',
      '- Beamer TeX may require running LaTeX from the repository root so `../figures/...` paths resolve from the `talk/` directory.',
      '- SVG/TikZ figure assets are stored under `/figures`.',
      '',
      '## Manifest',
      '',
      `See \`talk/${pkg.base || 'talk'}.manifest.json\`.`,
      ''
    ].join('\n');
  }

  function selectedPackageFiles(pkg, formats = selectedExportFormats()) {
    const assetFormats = figureAssetFormats();
    const files = [];

    for (const format of formats) {
      const file = pkg.files?.[format];
      if (file) files.push({ ...file, role: format });
    }

    const manifest = buildTalkManifest(pkg, formats);
    files.push({
      path: normalizePath(`talk/${pkg.base}.manifest.json`),
      content: JSON.stringify(manifest, null, 2) + '\n',
      mime: 'application/json',
      role: 'manifest'
    });
    files.push({
      path: normalizePath(`talk/README-${pkg.base}.md`),
      content: buildTalkReadme(pkg, formats),
      mime: 'text/markdown',
      role: 'readme'
    });
    ...
    for (const asset of pkg.figureAssets || []) {
      if (assetFormats.includes('svg')) {
        files.push({ path: asset.path, content: asset.svg, mime: 'image/svg+xml', role: 'figure-svg', asset });
      }
      if (assetFormats.includes('tikz')) {
        files.push({ path: asset.tikzPath, content: asset.tikz, mime: 'application/x-tex', role: 'figure-tikz', asset });
      }
    }
  ...
  function hasUnsafeSvg(svg) {
    const s = String(svg || '');
    return !/^<svg[\s\S]*<\/svg>\s*$/i.test(s.trim()) || /<script\b/i.test(s) || /\bon\w+\s*=/i.test(s) || /<foreignObject\b/i.test(s) || /href\s*=\s*["']https?:\/\//i.test(s);
  }

  function hasUnsafeTikz(tikz) {
    const s = String(tikz || '');
    return !/\\begin\{tikzpicture\}[\s\S]*\\end\{tikzpicture\}/.test(s) || /\\(write18|input|include|openout|read|catcode)\b/.test(s.replace(/\\begin\{tikzpicture\}[\s\S]*?\\end\{tikzpicture\}/, ''));
  }

  function validateTalkPackage(pkg, formats = selectedExportFormats()) {
    const errors = [];
    const warnings = [];
    const files = selectedPackageFiles(pkg, formats);
    const paths = new Set(files.map((file) => normalizePath(file.path)));

    if (!pkg?.ok) errors.push(pkg?.error || 'Package build failed.');
    if (!formats.length) errors.push('No export formats selected.');
    for (const format of formats) {
      if (!pkg.files?.[format]) errors.push(`Selected format missing from package: ${format}`);
    }

    const deckReport = validatePresentationDeck(pkg.deck, pkg.deck);
    if (!deckReport.ok) errors.push(`Deck validation failed: ${deckReport.errors.join('; ')}`);
    if (deckReport.warnings.length) warnings.push(...deckReport.warnings.map((w) => `Deck warning: ${w}`));

    const assetFormats = figureAssetFormats();
    const figureAssets = pkg.figureAssets || [];

    if (formats.includes('html')) {
      for (const asset of figureAssets) {
        if (assetFormats.includes('svg') && !paths.has(normalizePath(asset.path))) {
          errors.push(`HTML references missing SVG asset: ${asset.path}`);
        }
        if (asset.relFromTalk && !String(pkg.files.html?.content || '').includes(asset.relFromTalk)) {
          warnings.push(`HTML may not reference figure asset ${asset.relFromTalk}`);
        }
      }
    }

    if (formats.includes('beamer')) {
      const beamer = String(pkg.files.beamer?.content || '');
      for (const pkgName of ['graphicx', 'tikz', 'amsmath']) {
        if (!new RegExp(`\\\\usepackage(?:\\[[^\\]]*\\])?\\{[^}]*${pkgName}[^}]*\\}`).test(beamer)) {
          warnings.push(`Beamer file may be missing package: ${pkgName}`);
        }
      }
      for (const asset of figureAssets) {
        if (assetFormats.includes('tikz') && !paths.has(normalizePath(asset.tikzPath))) {
          errors.push(`Beamer references missing TikZ asset: ${asset.tikzPath}`);
        }
        if (assetFormats.includes('tikz') && asset.tikzRelFromTalk && !beamer.includes(asset.tikzRelFromTalk)) {
          warnings.push(`Beamer may not reference TikZ asset ${asset.tikzRelFromTalk}`);
        }
      }
    }

    for (const file of files) {
      if (!cleanText(file.path)) errors.push(`Package has a file without path (${file.role || 'unknown'}).`);
      if (file.content == null || String(file.content).length === 0) errors.push(`Package file is empty: ${file.path}`);
    }

    for (const asset of figureAssets) {
      if (assetFormats.includes('svg') && hasUnsafeSvg(asset.svg)) errors.push(`Unsafe or invalid SVG asset: ${asset.path}`);
      if (assetFormats.includes('tikz') && hasUnsafeTikz(asset.tikz)) errors.push(`Unsafe or invalid TikZ asset: ${asset.tikzPath}`);
    }

    const duplicatePaths = files.map((f) => normalizePath(f.path)).filter((path, i, arr) => arr.indexOf(path) !== i);
    if (duplicatePaths.length) errors.push(`Duplicate package file paths: ${[...new Set(duplicatePaths)].join(', ')}`);
    ...
    return {
      ...
      summary: {
        ...
        figureCount: figureAssets.length,
        errorCount: errors.length,
        warningCount: warnings.length
      }
    };
  ...
  function formatTalkPackageReport(report) {
    if (!report) return 'No talk package validation report yet.';
    const lines = [
      'Talk package validation report',
      '==============================',
      '',
      `Selected formats: ${(report.summary.selectedFormats || []).join(', ') || '(none)'}`,
      `Files: ${report.summary.fileCount}`,
      `Figures: ${report.summary.figureCount}`,
      `Errors: ${report.summary.errorCount}`,
      `Warnings: ${report.summary.warningCount}`,
      ''
    ];
    ...
    report.files.forEach((file) => lines.push(`- ${file.path} (${file.role || file.mime || 'file'})`));
    lines.push('');

    lines.push(report.ok ? 'Talk package passed validation.' : 'Talk package has errors; fix before sharing/compiling.');
    return lines.join('\n');
  }

  async function buildAndValidateTalkPackage() {
    const pkg = await buildTalkPackage(lastDeck);
    if (!pkg.ok) {
      setStatus(pkg.error || 'Could not build talk package.');
      if (pkg.report) setOutput(formatValidationReport(pkg.report));
      return pkg;
    }

    lastTalkPackage = pkg;
    const report = validateTalkPackage(pkg, selectedExportFormats());
    lastTalkPackageReport = report;
    setOutput(formatTalkPackageReport(report));
    setStatus(report.ok
      ? `Talk package built and validated with ${report.files.length} file(s).`
      : `Talk package has ${report.errors.length} error(s).`);
    return { ok: report.ok, package: pkg, report };
  }

  async function validateCurrentTalkPackage() {
    if (!lastTalkPackage) return buildAndValidateTalkPackage();
    const report = validateTalkPackage(lastTalkPackage, selectedExportFormats());
    lastTalkPackageReport = report;
    setOutput(formatTalkPackageReport(report));
    setStatus(report.ok
      ? 'Talk package passed validation.'
      : `Talk package has ${report.errors.length} error(s).`);
    return { ok: report.ok, package: lastTalkPackage, report };
  }
  ...
  function createZipBlob(files) {
    ...
    for (const file of files) {
      const nameBytes = encoder.encode(normalizePath(file.path));
      const dataBytes = encoder.encode(String(file.content ?? ''));
      const crc = crc32(dataBytes);

      const local = [];
      pushU32(local, 0x04034b50);
      pushU16(local, 20);
      pushU16(local, 0x0800); // UTF-8
      pushU16(local, 0); // store, no compression
      ...
      pushU16(cent, 0x0800);
      pushU16(cent, 0);
      pushU16(cent, now.dosTime);
      pushU16(cent, now.dosDate);
      pushU32(cent, crc);
      pushU32(cent, dataBytes.length);
      pushU32(cent, dataBytes.length);
      pushU16(cent, nameBytes.length);
      pushU16(cent, 0);
      pushU16(cent, 0);
      pushU16(cent, 0);
      pushU16(cent, 0);
  ...
  async function downloadTalkPackageZip() {
    let pkg = lastTalkPackage;
    if (!pkg) {
      const built = await buildAndValidateTalkPackage();
      if (!built?.package) return built;
      pkg = built.package;
    }
    ...
    if (!report.ok) {
      setOutput(formatTalkPackageReport(report));
      setStatus('Talk package has validation errors; ZIP not downloaded.');
      return { ok: false, package: pkg, report };
    }

    const blob = createZipBlob(report.files);
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${pkg.base || 'talk'}-package.zip`;
    ...
    setOutput(formatTalkPackageReport(report));
    setStatus(`Downloaded talk package ZIP with ${report.files.length} file(s).`);
    return { ok: true, package: pkg, report };
  }
  ...
  async function addSelectedExportsToGit() {
    ...
    if (!report.ok) {
      setOutput(formatTalkPackageReport(report));
      setStatus('Talk package has validation errors; not added to project.');
      return { ok: false, package: pkg, report };
    }

    const written = [];
    for (const file of report.files) written.push(writeProjectFile(file.path, file.content));

    lastDeck = pkg.deck;
    try { State()?.setActivePath?.(written.find((p) => /^talk\//.test(p)) || written[0] || 'talk'); } catch (_err) {}
    try { NS.Editor?.render?.(); } catch (_err) {}
    try { NS.FileTree?.render?.(); } catch (_err) {}
    try { State()?.save?.(); } catch (_err) {}

    setOutput([
      'Talk package files added to project',
      '===================================',
      '',
      ...written.map((p) => `- ${p}`),
      '',
      formatTalkPackageReport(report)
    ].join('\n'));
    setStatus(`Added validated talk package with ${written.length} file(s) to project.`);
    toast('Talk package added.');
    return { ok: true, written, package: pkg, report };
  }
  ...
  async function downloadSelectedExports() {
    ...
    if (!report.ok) {
      setOutput(formatTalkPackageReport(report));
      setStatus('Talk package has validation errors; individual files not downloaded.');
      return { ok: false, package: pkg, report };
    }

    let count = 0;
    for (const file of report.files) {
      if (file.role === 'manifest' || file.role === 'readme') continue;
    ...
    return { ok: true, downloaded: count, package: pkg, report };
  }
  ...
  function createCard() {
    ...
    card.innerHTML = [
      ...
      '  <div class="presentation-export-help">Stage 13G exports Presentation Maker compatible JSON. Slides now use <code>leftBlocks</code>/<code>rightBlocks</code> so content appears when imported, not only titles.</div>',
      ...
      '    <button id="buildTalkPackageBtn" class="btn mini" type="button">Build package</button>',
      '    <button id="validateTalkPackageBtn" class="btn mini" type="button">Validate package</button>',
      '    <button id="downloadTalkPackageZipBtn" class="btn mini primary" type="button">Download package ZIP</button>',
      '    <button id="addTalkExportsBtn" class="btn mini primary" type="button">Add package to /talk</button>',
      '    <button id="downloadTalkExportsBtn" class="btn mini" type="button">Download selected files</button>',
      '    <button id="runAddTalkExportsBtn" class="btn mini primary" type="button">Run + add package</button>',
      '    <button id="runDownloadTalkExportsBtn" class="btn mini" type="button">Run + download files</button>',
  ...
  function bindControls() {
    ...
    el('buildTalkPackageBtn')?.addEventListener('click', buildAndValidateTalkPackage, true);
    el('validateTalkPackageBtn')?.addEventListener('click', validateCurrentTalkPackage, true);
    el('downloadTalkPackageZipBtn')?.addEventListener('click', downloadTalkPackageZip, true);
  ...
  NS.PresentationExportService = {
    ...
    validateTalkPackage,
    formatTalkPackageReport,
    buildAndValidateTalkPackage,
    validateCurrentTalkPackage,
    createZipBlob,
    downloadTalkPackageZip,
    buildTalkPackage,
    renderDeckHtml,
    renderDeckBeamer,
    ...
    getLastTalkPackage: () => lastTalkPackage,
    getLastTalkPackageReport: () => lastTalkPackageReport
  };
