...
${details.output || ''}`),
  ...
  function fallbackPrompt() {
    return [
      ...
      'The final synthesis should include LATEXAI_BLOCK_PATCH blocks for source edits. Do not emit JSON edit schemas or \\lai/\\laiold markup; Latexai will add visible change markup after safe validation.'
    ].join('\n');
  }
  ...
  async function runDebate() {
    ...
    try {
      ...
      const synthInstructions = [
        ...
        'For every concrete source edit, include one or more LATEXAI_BLOCK_PATCH blocks. Do not return JSON edit schemas and do not output \\lai or \\laiold; the app/backend owns all visible change markup.',
        'Use insert_before_section for a newly requested front/middle section, append_before_end_document for appendices/end-of-paper material, and insert_after_block/insert_before_block only with listed safe anchors.',
        'Each BEGIN_NEW_LATEX body must be compile-safe visible LaTeX/prose: no Markdown fences, no preamble commands, no \\documentclass, no \\usepackage, no \\begin{document}/\\end{document}, balanced braces/environments, and text-mode special characters escaped.',
        'If a suggestion cannot be localized safely, use append_before_end_document with a concise section instead of fabricating an oldText or replacing the whole document.'
      ].filter(Boolean).join('\n');
