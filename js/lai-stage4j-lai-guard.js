/* Latexai Stage 4J always-visible LAI rewrite guard
 * Stage: stage4j-always-visible-lai-guard-1
 *
 * Purpose:
...
(function () {
  ...
  const STAGE = 'stage4j-always-visible-lai-guard-1';
  ...
  function setBadge(text, color) {
    let badge = document.getElementById('laiStage4jBadge');
    if (!badge) {
      badge = document.createElement('div');
      badge.id = 'laiStage4jBadge';
      badge.style.cssText = [
        'position:fixed',
        'right:12px',
        'top:12px',
    ...
    badge.textContent = text || 'LAI Guard Stage 4J active';
    if (color === 'green') {
  ...
  function rememberSelection(source) {
    ...
    if (cap) {
      pending = cap;
      setBadge('LAI Guard 4J: selection captured', 'blue');
    }
    return cap;
  }
...
`;
  ...
  function applyBlock(cap, replacement, reason) {
    ...
    try {
      ...
      if (out) out.textContent = `Stage 4J applied with \\lai{...} (${reason}). Old source was commented and replacement was wrapped.`;

      setBadge('Stage 4J applied \\lai{...}', 'green');
      NS.Main?.toast?.('Stage 4J applied rewrite with \\lai{...}.');
      pending = null;
      return true;
    } finally {
      applying = false;
  ...
  function installEditorPatches() {
    if (!NS.Editor || NS.Editor.__stage4jPatched) return;
    ...
    NS.Editor.__stage4jPatched = true;
  }

  function installCopilotPatch() {
    if (!NS.Copilot || NS.Copilot.__stage4jPatched) return;
    ...
    NS.Copilot.__stage4jPatched = true;
  }

  function installDomHandlers() {
    const el = editor();
    if (el && !el.__stage4jSelectionHandlers) {
      ...
      el.__stage4jSelectionHandlers = true;
    }

    const ask = document.getElementById('askCopilotBtn');
    if (ask && !ask.__stage4jAsk) {
  ...
  function installOutputObserver() {
    const out = document.getElementById('copilotOutput');
    if (!out || out.__stage4jObserver) return;
    ...
    out.__stage4jObserver = obs;
  }

  function boot() {
    if (document.body) setBadge('LAI Guard Stage 4J active', 'red');
    W.__LATEXAI_STAGE4J_LAI_GUARD_ACTIVE = true;
