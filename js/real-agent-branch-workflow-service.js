...
(function () {
  ...
  const STAGE = 'stage19t2m-generic-appendix-request-coverage-20260530-1';
...
<span id="status" class="sub"></span></header><main id="log"></main></body></html>`;
  ...
  function applySectionScopeToSelection(data) {
    ...
    try {
      ...
      if (equationCoverageActive() || appendixRequestActive()) {
        const eqTargets = selectedEquationTargets(data?.realAgentRunPayload || data || {});
        const mathTitle = equationCoverageActive() && appendixRequestActive() ? 'Explain mathematical equations and add requested appendix' : (equationCoverageActive() ? 'Explain mathematical equations and derivation steps' : 'Add requested appendix');
        const mathHint = [
  ...
  function planPayload() {
    ...
    const query = queryBase + (sectionScope === 'branch' ? '' : ' multi-section section-aware whole-paper revision') + (equationCoverageActive() ? ' equation explanation derivation below each equation' : '') + (appendixRequestActive() ? ' appendix append_before_end_document requested appendix coverage' : '');
    const reviewText = inputValue('branchWorkflowReviewText', queryBase) + coverageNote + equationNote + appendixNote;
